...
int I2Cstate=1; // If I2Cstate!=0, I2C read or write operation has failed
...
class MPU9250 {
    ...
    void writeByte(uint8_t address, uint8_t subAddress, uint8_t data)
    {
        ...
        data_write[1] = data;
        I2Cstate = i2c.write(address, data_write, 2, 0);
    }
    ...
    void readBytes(uint8_t address, uint8_t subAddress, uint8_t count, uint8_t * dest) // count=nº of bytes to read / dest=destiny where data is stored
    {     
        char data[14];
        char data_write[1];
        data_write[0] = subAddress;
        I2Cstate = i2c.write(address, data_write, 1, 1); // no stop
        I2Cstate = i2c.read(address, data, count, 0);
